...
const Card = ({ Stat, Label, Color }) => {
    ...
    return (
        <div className={`bg-white p-5 rounded-lg shadow-md transition duration-300 hover:shadow-lg border-t-4 ${colorClasses[Color]}`}>
...
const AdminPage = () => {
    ...
    const renderAdminContent = () => {
        const getTitle = (view) => view.replace('_', ' ').toLowerCase().split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ');
        
        const getDescription = (view) => {
             switch (view) {
                 case "ADMIN_DASHBOARD": return "Ringkasan aktivitas dan metrik kinerja utama sistem SkyLearn.";
                 case "TURUNAN": return "Kalkulator dan materi untuk perhitungan Turunan (Derivatif).";
                 case "ALJABAR": return "Alat bantu dan panduan untuk masalah Aljabar kompleks.";
                 case "TRIGONOMETRI": return "Penjelasan mendalam dan kalkulator untuk fungsi Trigonometri.";
                 case "LIMIT": return "Visualisasi dan pemecahan masalah Limit fungsi.";
                 case "INTEGRAL_LUAS_DAERAH": return "Kalkulator Integral untuk menghitung Luas Daerah di bawah kurva.";
                 default: return `Halaman untuk materi ${getTitle(currentAdminView)} di sini.`;
             }
        };
                ...
                <div className="bg-white p-8 rounded-xl shadow-lg w-full">
                    ...
                    {(() => {
                        switch (currentAdminView) {
                            ...
                            default:
                                return <p className="text-gray-600">Pilih menu dari Sidebar untuk memulai.</p>;
                        }
                    })()}
                </div>
            </>
