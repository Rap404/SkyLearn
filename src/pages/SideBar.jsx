...
const SideBar = ({ currentView, onNavigate, isOpen, toggleSidebar }) => {
  ...
  const NavItem = ({ item }) => {
    ...
    const baseClasses =
      "px-4 py-3 text-sm font-semibold cursor-pointer flex items-center transition duration-200 ease-in-out";
      ...
      >
        ...
        <div className="p-4 text-3xl font-extrabold text-white bg-indigo-800 flex justify-between items-center h-16 shadow-lg">
          <span className="tracking-wider flex items-center">
            <span className="text-indigo-300">Sky</span>
            <span className="text-white">Learn</span>
          </span>
          <button
            onClick={toggleSidebar}
            className="text-indigo-300 md:hidden p-1 rounded hover:bg-indigo-700"
          >
            <svg
        ...
        <nav className="mt-2 overflow-y-auto h-[calc(100vh-64px)] pb-4">
          ...
          <p className="text-xs font-semibold uppercase text-indigo-400 mx-4 mb-2 tracking-widest">
            Materi Kalkulus
          </p>
